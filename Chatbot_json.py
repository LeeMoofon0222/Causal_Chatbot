...
def model_select(model):
    if model == "Llama3 8b":
        ...
        model_type = "llama"
        exclude_component_id = "OpenAIModel-jz1Db"
    elif model == "GPT 4o":
        model_name = "gpt-4o"
        model_type = "openai"
        exclude_component_id = "OllamaModel-0kgkQ"
    elif model == "GPT 4o mini":
        model_name = "gpt-4o-mini"
        model_type = "openai"
        exclude_component_id = "OllamaModel-0kgkQ"
    return model_name, model_type, exclude_component_id
...
def load_and_modify_flow(flow_path, exclude_component_id):

    with open(flow_path, "r", encoding="utf-8") as f:
        flow_data = json.load(f)
    flow_data["data"]["nodes"] = [
        node for node in flow_data["data"]["nodes"] if node["id"] != exclude_component_id
    ]
    print(flow_data)
    return flow_data

def run_modified_flow(flow_path, exclude_component_id, input_value, TWEAKS):
    modified_flow_data = load_and_modify_flow(flow_path, exclude_component_id)

    result = run_flow_from_json(
        flow=modified_flow_data,
...
flow_path = "json_file_storage/RAG Based LLM_Chatbot.json"
...
model_name, model_type, exclude_component_id = model_select(model)
...
if prompt:
    ...
    with st.chat_message("assistant"):
        ...
        input_type = "chat"
        try:
            #response = run_modified_flow(flow_path, exclude_component_id, prompt, TWEAKS)
            try:
                # response = run_modified_flow(flow_path,
